...
def flytoplace(distance, difficultyValue, nroOfSosigs, mods):
    ...
    #if the modifier 'norandom' is on, then death will only occur if the chance is atleast 100% or more
    ...
    if mods["norandom"]:
        if chance >= 1000:
            confirm = input(
                f"Flying to this airport will result in death, are you sure you want to continue? (y/n): ").lower()
        else:
            return "success"
    elif chance >= 0:
